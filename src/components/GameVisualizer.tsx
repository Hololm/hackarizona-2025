...
const GameVisualizer: React.FC<GameVisualizerProps> = ({ className }) => {
  ...
  if (isLoading || !gameState) {
    return (
      <div className={cn('glass-panel rounded-xl p-6 animate-pulse min-h-[300px] flex items-center justify-center', className)}>
        <div className="text-center text-muted-foreground">Loading game visualizer...</div>
      </div>
    );
  }

  const getTableStateClass = () => {
    if (gameState.status === 'waiting') return 'bg-neutral-100/80 dark:bg-neutral-900/80';
    if (gameState.status === 'completed') {
      if (gameState.result === 'win') return 'bg-neutral-100/80 dark:bg-neutral-900/80';
      if (gameState.result === 'lose') return 'bg-neutral-100/80 dark:bg-neutral-900/80';
      if (gameState.result === 'push') return 'bg-neutral-100/80 dark:bg-neutral-900/80';
    }
  ...
  return (
    <div className={cn(
      ...
      <div className="p-3 bg-neutral-100 dark:bg-neutral-900 border-b border-neutral-200 dark:border-neutral-800 flex justify-between items-center">
        <h2 className="text-sm font-medium text-neutral-800 dark:text-neutral-200">Live Blackjack Table</h2>
      ...
      <div className={cn(
        ...
        <div className="relative w-full h-[320px] mt-8">
          ...
          <div className="absolute top-0 left-1/2 transform -translate-x-1/2 w-full text-center z-10">
            <div className="text-xs text-neutral-500 dark:text-neutral-400 mb-1 font-medium">DEALER</div>
          ...
          <div className="absolute top-[60px] left-0 w-[40%]">
            {otherPlayerHands.slice(0, 3).map((hand, playerIndex) => (
              <div 
                ...
                className="absolute"
                style={{
                  top: `${playerIndex * 60}px`,
                  left: `${10 + playerIndex * 15}px`,
                }}
              >
                <div className="text-[10px] text-neutral-500 dark:text-neutral-400 mb-1">PLAYER {playerIndex + 1}</div>
          ...
          <div className="absolute top-[60px] right-0 w-[40%]">
            {otherPlayerHands.slice(3, 6).map((hand, playerIndex) => (
              <div 
                ...
                style={{
                  top: `${playerIndex * 60}px`,
                  right: `${10 + playerIndex * 15}px`,
                }}
              >
                <div className="text-[10px] text-neutral-500 dark:text-neutral-400 mb-1 text-right">PLAYER {playerIndex + 4}</div>
          ...
          <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-20">
            <div className="py-2 px-5 rounded-full bg-neutral-100/30 dark:bg-neutral-900/30 backdrop-blur-sm shadow-sm border border-neutral-300/20 dark:border-neutral-700/20 text-center">
              {gameState.status === 'waiting' && (
                <div className="text-xs text-neutral-600 dark:text-neutral-300">PLACE YOUR BET</div>
              )}
              {gameState.status === 'playing' && (
                <div className="text-xs text-neutral-600 dark:text-neutral-300 font-medium">IN PLAY</div>
              )}
              {gameState.status === 'completed' && (
                <div className={cn(
                  'text-xs font-medium',
                  gameState.result === 'win' ? 'text-green-600 dark:text-green-400' : 
                  gameState.result === 'lose' ? 'text-red-600 dark:text-red-400' : 
          ...
          <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 w-full text-center z-10">
            ...
            <div className="text-xs text-neutral-600 dark:text-neutral-300 mt-2 font-medium">YOU</div>
          </div>
        </div>
      </div>
    </div>
